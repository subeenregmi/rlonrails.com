...
const MAX_VIEW_W = MAP_BOUNDS.w * 1.15;
...
const clampView = (v: View) => {
  const minX = MAP_BOUNDS.x - PAN_MARGIN;
  const maxX = MAP_BOUNDS.x + MAP_BOUNDS.w + PAN_MARGIN;
  const minY = MAP_BOUNDS.y - PAN_MARGIN;
  const maxY = MAP_BOUNDS.y + MAP_BOUNDS.h + PAN_MARGIN;
  v.x = v.w >= maxX - minX ? (minX + maxX - v.w) / 2 : clamp(v.x, minX, maxX - v.w);
  v.y = v.h >= maxY - minY ? (minY + maxY - v.h) / 2 : clamp(v.y, minY, maxY - v.h);
};
...
const wrapIntervals = (from: number, head: number, length: number): Interval[] =>
  head <= length ? [[from, head]] : [[from, length], [0, head - length]];
...
const Track = memo(function Track({ line, layout, schedule, colour, dash, frontier, readEnds, dim }: TrackProps) {
  const l = layout.lines[line.id];
  ...
  return (
    <>
      ...
      <path className="track-colour dimmable" data-dim={dim} d={l.d} stroke={colour} strokeDasharray={dash} mask={`url(#draw-${line.id})`} />
      {frontier && <path ref={lockFlowPhase} className="track-next dimmable" data-dim={dim} d={frontier} stroke={colour} mask={`url(#draw-${line.id})`} />}
    </>
  );
});
...
const LineStations = memo(function LineStations({ line, layout, schedule, colour, statusKey, nextId, hereId, selectedId, dim, register }: LineStationsProps) {
  return (
    <g>
...
const LineLabels = memo(function LineLabels({ line, layout, schedule, colour, statusKey, nextId, selectedId, dim }: Omit<LineStationsProps, "register">) {
  return (
    <g>
      {line.stations.map((station, index) => {
        const s = layout.stations[station.id];
...
const LineBooths = memo(function LineBooths({ line, layout, schedule, statusKey, dim }: Omit<LineStationsProps, "register" | "colour" | "nextId" | "selectedId">) {
  return (
    <g>
      {line.stations.map((station, index) => {
...
const LineLamps = memo(function LineLamps({ line, layout, schedule, statusKey, dim }: Omit<LineStationsProps, "register" | "colour" | "nextId" | "selectedId">) {
  return (
    <g>
      {line.stations.map((station, index) => {
...
export const TubeMap = forwardRef<TubeMapHandle, TubeMapProps>(function TubeMap(
  { layout, schedule, hereId, nextIds, completeIds, progress, colours, selectedId, focusLineId, trainCount, onSelect, onPinLine },
  ref,
) {
  ...
  const listeners = useRef(new Set<(view: View) => void>());
  ...
  const render = useCallback((commitSoon: boolean) => {
    ...
    if (!flightRef.current && (nearEdge || zoomDrift) && performance.now() - lastCommit.current > EAGER_COMMIT_MS) { commit(); return; }
    if (commitSoon) {
      if (commitTimer.current) clearTimeout(commitTimer.current);
      commitTimer.current = setTimeout(commit, COMMIT_DELAY);
    }
  }, [commit, wrapRect]);

  const setViewCentered = useCallback((cx: number, cy: number, w: number, commitSoon = true) => {
    const v = pending.current;
    v.w = clamp(w, MIN_VIEW_W, MAX_VIEW_W);
    v.h = v.w / aspect();
    v.x = cx - v.w / 2;
    v.y = cy - v.h / 2;
    render(commitSoon);
  ...
  const flyTo = useCallback((cx: number, cy: number, w: number, duration = 850) => {
    ...
    const target = { cx, cy, w: clamp(w, MIN_VIEW_W, MAX_VIEW_W) };
  ...
  }, [setViewCentered, commit]);

  const fitAll = useCallback((duration?: number) => {
    const w = Math.max(MAP_BOUNDS.w, MAP_BOUNDS.h * aspect());
    flyTo(MAP_BOUNDS.x + MAP_BOUNDS.w / 2, MAP_BOUNDS.y + MAP_BOUNDS.h / 2, w, duration);
  }, [flyTo, aspect]);
  ...
  useEffect(() => {
    ...
    measureWrap();
    setViewCentered(MAP_BOUNDS.x + MAP_BOUNDS.w / 2, MAP_BOUNDS.y + MAP_BOUNDS.h / 2, Math.max(MAP_BOUNDS.w, MAP_BOUNDS.h * aspect()) * 1.08, false);
    commit();
    fitAll(2400);
    const timer = setTimeout(() => flyTo(START_VIEW.cx, START_VIEW.cy, Math.max(START_VIEW.w, START_VIEW.h * aspect()), 1800), Math.max(3000, schedule.total - 1200));
    return () => {
      observer.disconnect();
  ...
  }, [setViewCentered, commit, fitAll, flyTo, aspect, measureWrap, schedule.total]);
  ...
  const animateSegment = useCallback((line: Line, fromPos: number, toPos: number, done: () => void) => {
    ...
    const step = (now: number) => {
      ...
      const dash = dashArray(wrapIntervals(fromPos, headPos, lineLayout.length).sort((a, b) => a[0] - b[0]), lineLayout.length);
  ...
  const growFrontier = useCallback((line: Line, fromPos: number, toPos: number, done: () => void) => {
    ...
    const step = (now: number) => {
      const k = easeInOut(Math.min(1, (now - start) / duration));
      dashed.setAttribute("d", intervalsPath(lineLayout, wrapIntervals(fromPos, fromPos + distance * k, lineLayout.length)));
      if (k < 1) { requestAnimationFrame(step); return; }
      done();
  ...
  useEffect(() => {
    ...
    fresh.forEach((id) => {
      ...
      const prev = line.stations[index - 1] ?? (line.closed ? line.stations[line.stations.length - 1] : null);
      ...
      const reveal = () => {
        ...
        const grows = CURRICULUM.lines.flatMap((track) => {
          const nodes = trackNodes(track, layout);
          const at = nodes.findIndex((n) => n.id === id);
          const after = at < 0 ? null : nodeAfter(track, nodes, at);
          if (!after || isRead(progress, after.id)) return [];
          const from = nodes[at].pos;
          return [{ track, from, to: after.pos > from ? after.pos : layout.lines[track.id].length + after.pos }];
        });
        if (!grows.length) { finish(); return; }
        let pending = grows.length;
        const oneDone = () => { if (--pending === 0) finish(); };
        grows.forEach((g) => growFrontier(g.track, g.from, g.to, oneDone));
      };
      if (!prev || !isRead(progress, prev.id)) { reveal(); return; }
      const from = layout.stations[prev.id].pos;
      const own = layout.stations[id].pos;
      animateSegment(line, from, own <= from ? layout.lines[line.id].length + own : own, reveal);
    });
  ...
  const { intervalsByLine, frontierByLine } = useMemo(() => {
    ...
    for (const line of CURRICULUM.lines) {
      ...
      nodes.forEach((node, i) => {
        if (!isRead(progress, node.id)) return;
        ...
        if (isRead(progress, next.id)) {
          if (animating.get(next.id) === "fill") dashed.push(...segments);
          else list.push(...segments);
        } else if (!animating.has(node.id)) {
  ...
  const subscribe = useCallback((fn: (view: View) => void) => {
  ...
  const onPointerMove = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (pinch) {
      ...
      zoomAt(pinch.dist / now.dist, now.mid.x, now.mid.y);
    ...
    if (d) {
      ...
      render(false);
      if (performance.now() - lastCommit.current > DRAG_COMMIT_MS) commit();
      return;
    }
  ...
  const releasePointer = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (pointersRef.current.size === 0) { pinchRef.current = null; setDragging(false); commit(); }
    return true;
  };
  const onPointerUp = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    setDragging(false);
    if (!pressed) return;
    if (pressed.moved) { commit(); return; }
  ...
  return (
    <>
      <svg
        ref={svgRef}
        className={cx("map-svg absolute", dragging && "dragging")}
        ...
        onPointerCancel={(e) => { if (releasePointer(e)) return; dragRef.current = null; setDragging(false); commit(); }}
      ...
      >
        ...
        <g>
          {CURRICULUM.lines.map((line) => (
            <Track key={line.id} line={line} layout={layout} schedule={schedule} colour={colours[line.id]} dash={dashByLine[line.id]} frontier={frontierByLine[line.id]} readEnds={readEndsByLine[line.id]} dim={dim(line.id)} />
          ))}
        </g>
        <g ref={animLayerRef} />
        <g>
          {CURRICULUM.lines.map((line) => (
            <LineLamps key={line.id} line={line} layout={layout} schedule={schedule} statusKey={statusKeys[line.id]} dim={dim(line.id)} />
          ))}
        </g>
        <g>
          {CURRICULUM.lines.map((line) => (
            <LineBooths key={line.id} line={line} layout={layout} schedule={schedule} statusKey={statusKeys[line.id]} dim={dim(line.id)} />
          ))}
        </g>
        <g className="trains-layer" style={{ "--delay": `${schedule.total.toFixed(0)}ms` } as React.CSSProperties}>
          <Trains layout={layout} intervals={intervalsByLine} count={trainCount} subscribe={subscribe} />
        </g>
      ...
      <Minimap layout={layout} surface={surface} colours={colours} activeLines={activeLines} subscribe={subscribe} onJump={(x, y) => { setViewCentered(x, y, pending.current.w, false); commit(); }} />
    </>
  );
});
