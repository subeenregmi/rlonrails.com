...
const tool =
  "rounded-full bg-tint px-3 py-1.5 text-[13px] text-ink transition hover:bg-tint-strong active:translate-y-px";
...
const query = () => {
  compactQuery ??= globalThis.matchMedia(COMPACT);
  return compactQuery;
};
...
export function FloatingBar(props: FloatingBarProps) {
  ...
  return (
    <div
      ref={rootRef}
      className={cx(
        "floating-bar absolute top-[calc(1rem+var(--safe-top))] left-[calc(1rem+var(--safe-left))] max-w-[calc(100%-2rem-var(--safe-left)-var(--safe-right))]",
        modal ? "z-30" : "z-20",
        barOpen && "bar-open",
      )}
    >
              ...
              <div className="hidden h-2 w-16 overflow-hidden rounded-full bg-white/20 sm:w-24 min-[400px]:block">
                <div
                  className={cx(
                    ...
                    totals.routeRead >= totals.routeTotal && "rainbow",
                  )}
                  style={{ width: `${(100 * totals.routeRead) / totals.routeTotal}%` }}
                />
              </div>
              <span className="whitespace-nowrap">
                {totals.routeRead} / {totals.routeTotal}
              </span>
            </div>
            <button
              type="button"
      ...
      {open ? (
        <Shell modal={modal} onDismiss={fold}>
          ...
          <section
            className={cx(
              modal
                ...
                : "max-h-[40dvh] overflow-y-auto sm:relative sm:max-h-none sm:overflow-visible",
            )}
          >
            <div
              className={cx(
                "pr-1",
                modal ? "sm:h-full sm:overflow-y-auto" : "sm:absolute sm:inset-0 sm:overflow-y-auto",
              )}
            >
              <h2 className="mb-2 text-[11px] text-ink-soft uppercase tracking-[0.1em]">Lines</h2>
                  ...
                  return (
                    <li key={line.id}>
                      <button
                        ...
                        className={cx(
                          "grid w-full cursor-pointer grid-cols-[6px_1fr_auto] items-center gap-x-2.5 gap-y-1 rounded-lg px-2 py-1.5 text-left transition hover:bg-tint",
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex min-w-0 items-center gap-1.5 text-[13px] leading-tight">
                            ...
                            {tally.complete ? <Tick className="size-3.5 flex-none" style={{ color: colour }} /> : null}
                            {line.track && tracks.includes(line.id) && (
                              <span className="flex-none rounded-full bg-tint-strong px-1.5 py-px text-[9.5px] text-ink-soft uppercase tracking-[0.06em]">
                                Chosen
                              </span>
                            )}
                          </div>
          ...
          <section
            className={cx("flex flex-col gap-4", modal && "sm:min-h-0 sm:w-[200px] sm:flex-none sm:overflow-y-auto")}
          >
            <div>
              <h2 className="mb-2 text-[11px] text-ink-soft uppercase tracking-[0.1em]">Progress</h2>
              <dl className="grid grid-cols-[1fr_auto] gap-y-1 text-[12.5px]">
                <dt className="text-ink-soft">Core</dt>
                <dd className="text-right">
                  {totals.coreRead} / {totals.coreTotal}
                </dd>
                ...
                <dd className="text-right">
                  {totals.trackTotal > 0 ? (
                    `${totals.trackRead} / ${totals.trackTotal}`
                  ) : (
                    <span className="text-ink-faint">None picked</span>
                  )}
                </dd>
                <dt className="text-ink-soft">Exercises</dt>
                <dd className="text-right">
                  {totals.exerciseRead} / {totals.exerciseTotal}
                </dd>
                <dt className="text-ink-soft" title="Reference stations stay off every route">
                  Reference
                </dt>
                <dd className="text-right">
                  {totals.referenceRead} / {totals.referenceTotal}
                </dd>
                <dt className="text-ink-soft">Implemented</dt>
                <dd className="text-right">{totals.implemented}</dd>
                <dt className="text-ink-soft">Investigated</dt>
                <dd className="text-right">{totals.investigated}</dd>
                <dt className="text-ink-soft">Explored</dt>
                <dd className="text-right">
                  {totals.read} / {totals.total}
                </dd>
                <dt className="text-ink-soft">Resources</dt>
                <dd className="text-right">
                  {totals.resourcesDone} / {totals.resourcesTotal}
                </dd>
                <dt className="text-ink-soft">Trains running</dt>
                <dd className="text-right">{trainCount}</dd>
              </dl>
            </div>
            <div>
              <h2 className="mb-1 text-[11px] text-ink-soft uppercase tracking-[0.1em]">Your route</h2>
              <p className="mb-2 text-[11.5px] text-ink-faint leading-snug">
                Core stations and the exercises on the spine are for everyone. Specialisations are the rest: choose the
                ones you mean to do and their stations join your route. Reference stations stay off every route, so the
                route count is smaller than the map. The map asks once, after the research sampler.
              </p>
            ...
            <div>
              <h2 className="mb-2 text-[11px] text-ink-soft uppercase tracking-[0.1em]">Data</h2>
              <div className="flex flex-wrap gap-1.5">
                ...
                {process.env.NODE_ENV !== "production" && (
                  <button
                    ...
                    title="Mark every station and resource as read"
                  >
                    Select all
                  </button>
                )}
                <button
                  type="button"
                  className={`${tool} hover:bg-tfl-red hover:text-white`}
                  onClick={() => {
                    fold();
                    onReset();
                  }}
            ...
            <p className="text-[11px] text-ink-faint leading-snug">
              Scroll to pan, pinch or ⌘-scroll to zoom, drag to move. Arrow keys move along a line, space marks a
              station read.
            </p>
            <div className="mt-auto flex items-center justify-end gap-3 border-rule border-t pt-3 text-[11px] text-ink-faint">
              <span>
                Created by{" "}
                <a
                  href="https://subeenregmi.com"
                  target="_blank"
              ...
              <a
                href="https://github.com/subeenregmi/rlonrails.com"
                target="_blank"
                rel="noreferrer noopener"
                title="GitHub"
                className="flex size-7 items-center justify-center rounded-full bg-tint text-ink-soft hover:bg-tint-strong hover:text-ink"
              >
...
function Shell({ modal, onDismiss, children }: { modal: boolean; onDismiss: () => void; children: ReactNode }) {
  ...
  if (!modal) {
    return (
      <div
        className={cx(
          "panel-enter mt-2 grid max-h-[calc(100dvh-12.5rem-var(--safe-top)-var(--safe-bottom))] w-[560px] max-w-[calc(100vw-2rem-var(--safe-left)-var(--safe-right))] grid-cols-1 gap-4 overflow-y-auto overscroll-contain shadow-[0_18px_50px_rgba(0,0,0,.18)] sm:grid-cols-[1fr_200px]",
          box,
        )}
      >
        {children}
  ...
  return (
    ...
    >
      <div
        ...
        className={cx(
          "dialog-enter flex h-full w-[560px] max-w-full flex-col shadow-[0_24px_70px_rgba(0,0,0,.32)]",
          box,
        )}
      >
        ...
        <header className="mb-3 flex flex-none items-center justify-between">
          <h2 id={titleId} className="text-[11px] text-ink-soft uppercase tracking-[0.1em]">
            Menu
          </h2>
          <button
            type="button"
            ...
            className="-my-1 -mr-1 flex size-8 items-center justify-center rounded-full bg-tint text-ink-soft transition hover:bg-tint-strong hover:text-ink"
          >
