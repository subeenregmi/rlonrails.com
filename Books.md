...
- **The route is tiered, and the core of it is small.** On the map, a station is **Core** (everyone needs it), a **Track requirement** (needed for the specialisation you picked), **Reference** (fetch it when a project asks for it), or an **Exercise** (something you do, judged by deliverables rather than by ticking the reading). The default route is roughly 65 stations out of 151; picking a specialisation adds its own. The Essential/Deeper labels in the prose below are the older, coarser version of the same idea — where they disagree, the map's tier is the one that has been thought about most recently.
...
- **Deep-learning readiness (diagnostic, not a course).** Automatic differentiation, optimisers and their schedules, convolutional and recurrent nets, normalisation, and a training loop you have written yourself. *Dive into Deep Learning* (chs. 3–7) covers it; the PyTorch basics tutorial is the short version, CS231n if the vision half is the gap. **Core**, and test yourself the same way — Phase 2 asks you to put a convnet where the Q-table was and debug it when it does not learn, and nothing on this map teaches that part. An agent that will not learn is usually a broken implementation, which is only diagnosable if the deep-learning half is not also new to you.
...
This line assumes the language-model half rather than teaching it: tokenised sequences, pretraining, and supervised fine-tuning are used from the first entry onward. If SFT is not already a thing you have done, do that reading before this phase rather than during it.
...
- **RL with Verifiable Rewards (RLVR):** term popularized by **Lambert et al., "Tülu 3" (2024).** Use automatically checkable correctness (math answer/unit-test) as reward. **Essential concept**, and read before R1 rather than after it: it is the reward R1 is built on.
...
Keep models tiny (≤1.5B, e.g. a Qwen/DeepSeek-Distill-1.5B) so you can iterate. Christiano, InstructGPT, DPO, GRPO and RLVR are core — the preference-learning root and the reward definition are what make the rest legible; everything past them is a specialisation, and a thesis on exploration or continuous control does not need it.
