...
async function aim(page: Page) {
  const { dx, dy, transform } = await page.evaluate(() => {
    ...
    return {
      dx: here.x + here.width / 2 - (box.x + box.width / 2),
      dy: here.y + here.height / 2 - (box.y + box.height / 2),
      transform: document.querySelector<HTMLElement>(".here-pin-arrow")!.style.transform,
    };
  });
  const rotation = Number(ROTATION.exec(transform)![1]);
